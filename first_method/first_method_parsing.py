# This part of the compiler is responsible for parsing the input IDPZ3 program.
...
class ParsedIDPZ3Program:
    def __init__(self, types, predicates, functions, rules):
        self.types = types
...
def parse(idp_program):
    ...
    for transformed_interp in transformed_interpretations:
        if type(transformed_interp) == Type:
            ...
            transformed_rules.extend(get_structure_rules(transformed_interp))


    return ParsedIDPZ3Program(types, predicates, functions, transformed_rules)
